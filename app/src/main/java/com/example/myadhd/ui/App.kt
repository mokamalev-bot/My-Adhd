...
@Composable
fun AppRoot(taskVm: TaskViewModel, habitVm: HabitViewModel, preferencesVm: PreferencesViewModel) {
    val preferences by preferencesVm.preferences.collectAsState()
    if (!preferences.onboardingComplete) { Onboarding({ preferencesVm.completeOnboarding() }) { language -> preferencesVm.setLanguage(language); applyLanguage(language) }; return }
...
@Composable private fun Onboarding(onComplete: () -> Unit, onLanguage: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        Text(stringResource(R.string.welcome_title), style = MaterialTheme.typography.headlineLarge); Spacer(Modifier.height(12.dp)); Text(stringResource(R.string.welcome_subtitle), style = MaterialTheme.typography.bodyLarge); Spacer(Modifier.height(24.dp)); Text(stringResource(R.string.language), style = MaterialTheme.typography.titleMedium)
        ...
        Button({ onLanguage(language); onComplete() }, Modifier.fillMaxWidth()) { Text(stringResource(R.string.get_started)) }; TextButton({ onLanguage(language); onComplete() }, Modifier.fillMaxWidth()) { Text(stringResource(R.string.continue_guest)) }
    }
}

@Composable private fun HomeScreen(tasks: List<TaskEntity>, onTasks: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(24.dp)) { Text(stringResource(R.string.today), style = MaterialTheme.typography.headlineMedium); Text(stringResource(R.string.next_step), Modifier.padding(top = 8.dp)); val next = tasks.firstOrNull { !it.completed }; Card(Modifier.fillMaxWidth().padding(top = 20.dp)) { Column(Modifier.padding(20.dp)) { Text(next?.title ?: stringResource(R.string.no_tasks)); next?.let { Text(stringResource(R.string.minutes_format, it.estimatedMinutes)) } } }; Button(onTasks, Modifier.padding(top = 20.dp)) { Text(stringResource(R.string.view_tasks)) } }
}
...
@Composable private fun TasksScreen(vm: TaskViewModel) {
    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) { Text(stringResource(R.string.tasks), style = MaterialTheme.typography.headlineMedium); Row(Modifier.fillMaxWidth().padding(vertical = 16.dp)) { OutlinedTextField(title, { title = it }, Modifier.weight(1f), placeholder = { Text(stringResource(R.string.task_hint)) }, singleLine = true); Spacer(Modifier.width(8.dp)); Button({ vm.add(title); title = "" }) { Text(stringResource(R.string.add)) } }; tasks.forEach { task -> ListItem(headlineContent = { Text(task.title) }, supportingContent = { Text(stringResource(R.string.minutes_format, task.estimatedMinutes)) }, leadingContent = { Checkbox(task.completed) { vm.toggle(task) } }, trailingContent = { TextButton({ vm.delete(task) }) { Text(stringResource(R.string.delete)) } }) } }
}
...
@Composable private fun ProgressScreen(vm: HabitViewModel) {
    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) { Text(stringResource(R.string.progress), style = MaterialTheme.typography.headlineMedium); Text(stringResource(R.string.habits_completed, completed, habits.size), Modifier.padding(top = 8.dp)); Row(Modifier.fillMaxWidth().padding(vertical = 16.dp)) { OutlinedTextField(name, { name = it }, Modifier.weight(1f), placeholder = { Text(stringResource(R.string.habit_hint)) }, singleLine = true); Spacer(Modifier.width(8.dp)); Button({ vm.add(name); name = "" }) { Text(stringResource(R.string.add)) } }; habits.forEach { habit -> HabitRow(habit, today, vm) } }
}

@Composable private fun HabitRow(habit: HabitEntity, today: String, vm: HabitViewModel) { ListItem(headlineContent = { Text(habit.name) }, leadingContent = { Checkbox(habit.completedToday(today)) { vm.toggle(habit) } }, trailingContent = { TextButton({ vm.delete(habit) }) { Text(stringResource(R.string.delete)) } }) }

@Composable private fun ProfileScreen(darkMode: Boolean, language: String, onDarkModeChanged: (Boolean) -> Unit, onLanguageChanged: (String) -> Unit) { Column(Modifier.fillMaxSize().padding(24.dp)) { Text(stringResource(R.string.profile), style = MaterialTheme.typography.headlineMedium); Row(Modifier.fillMaxWidth().padding(top = 20.dp), horizontalArrangement = Arrangement.SpaceBetween) { Text(stringResource(R.string.dark_mode)); Switch(darkMode, onDarkModeChanged) }; Text(stringResource(R.string.language), Modifier.padding(top = 20.dp), style = MaterialTheme.typography.titleMedium); Row(Modifier.padding(top = 8.dp)) { FilterChip(language == "en", { onLanguageChanged("en") }, label = { Text(stringResource(R.string.language_english)) }); Spacer(Modifier.width(8.dp)); FilterChip(language == "ar", { onLanguageChanged("ar") }, label = { Text(stringResource(R.string.language_arabic)) }); Spacer(Modifier.width(8.dp)); FilterChip(language == "system", { onLanguageChanged("system") }, label = { Text(stringResource(R.string.language_system)) }) }; Text(stringResource(R.string.disclaimer), Modifier.padding(top = 24.dp)) } }
